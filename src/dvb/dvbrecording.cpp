	...
	manager(manager_), hasPendingOperation(false)
{
	...
	if (!file.open(QIODevice::ReadOnly)) {
		...
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::DvbRecordingModel: cannot open file %1", file.fileName())));
		return;
	}
	...
	while (!stream.atEnd()) {
		...
		if (stream.status() != QDataStream::Ok) {
			// xgettext:no-c-format
			qWarning("%s", qPrintable(i18n("DvbRecordingModel::DvbRecordingModel: invalid recordings in file %1", file.fileName())));
			break;
		}
	...
	if (!file.remove()) {
		// xgettext:no-c-format
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::DvbRecordingModel: cannot remove file %1", file.fileName())));
	}
}
...
DvbRecordingModel::~DvbRecordingModel()
{
	if (hasPendingOperation) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::~DvbRecordingModel: illegal recursive call")));
	}
...
DvbSharedRecording DvbRecordingModel::addRecording(DvbRecording &recording, bool checkForRecursion)
{
	if (checkForRecursion) {
		if (hasPendingOperation) {
			qWarning("%s", qPrintable(i18n("DvbRecordingModel::addRecording: illegal recursive call")));
	...
	if (!recording.validate()) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::addRecording: invalid recording")));
		return DvbSharedRecording();
	}
...
{
	if (hasPendingOperation) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::updateRecording: illegal recursive call")));
		return;
	}
	...
	if (!recording.isValid() || (recordings.value(*recording) != recording) ||
		...
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::updateRecording: invalid recording")));
		return;
	}
...
{
	if (hasPendingOperation) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::removeRecording: illegal recursive call")));
		return;
	}
	...
	if (!recording.isValid() || (recordings.value(*recording) != recording)) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::removeRecording: invalid recording")));
		return;
	}
...
void DvbRecordingModel::disableLessImportant(DvbSharedRecording &recording1, DvbSharedRecording &recording2)
{
	if (recording1->priority < recording2->priority) {
		...
		rec1.disabled = true;
		// xgettext:no-c-format
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::disableLessImportant disabled %1", recording1->name)));
	}
	if (recording2->priority < recording1->priority) {
		DvbRecording rec2 = *(recording1.constData());
		rec2.disabled = true;
		// xgettext:no-c-format
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::disableLessImportant disabled %1", recording2->name)));
	}
}
...
void DvbRecordingModel::addToUnwantedRecordings(DvbSharedRecording recording)
{
	...
	// xgettext:no-c-format
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::addToUnwantedRecordings executed %1", recording->name)));
}
...
void DvbRecordingModel::executeActionAfterRecording(DvbRecording recording)
{
	...
	if (!stopCommand.isEmpty())
	{
		...
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::shutdownWhenEmpty:could not execute cmd")));
	}
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::executeActionAfterRecording executed.")));
...
void DvbRecordingModel::removeDuplicates()
{
		...
		foreach(DvbSharedRecording rec2, recordingList)
		{
			...
			if (i < j) {
				if (loopEntry1.begin == loopEntry2.begin
					...
					// xgettext:no-c-format
					qWarning("%s", qPrintable(i18n("DvbRecordingModel::removeDuplicates removed. %1", loopEntry1.name)));
				}
			}
			j = j + 1;
		}
	...
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::removeDuplicates executed.")));
...
bool DvbRecordingModel::existsSimilarRecording(DvbEpgEntry recording)
{
	...
	foreach(DvbSharedRecording unwanted, unwantedRecordingsList)
	{
		...
		if (QString::compare(entry.begin.toString(), ((loopEntry.begin.addSecs(manager->getBeginMargin()))).toString()) == 0
			...
			// xgettext:no-c-format
			qDebug("%s", qPrintable(i18n("DvbRecordingModel::existsSimilarRecording Found from unwanteds  %1", loopEntry.name)));
			found = true;
			break;
		}
	}
	...
	foreach(DvbSharedRecording rec1, recordingList)
	{
		...
		foreach(DvbSharedRecording rec2, recordingList)
		{
			if (isInConflictWithAll(rec2, conflictList)) {
				...
				// xgettext:no-c-format
				qDebug("%s", qPrintable(i18n("DvbRecordingModel::disableConflicts: '%1' '%2' and '%3' '%4'", rec1->name, rec1->begin.toString(), rec2->name, rec2->begin.toString())));
...
DvbSharedRecording DvbRecordingModel::getLeastImportant(QList<DvbSharedRecording> recList)
{
	...
	{
		// xgettext:no-c-format
		qDebug("%s", qPrintable(i18n("DvbRecordingModel::getLeastImportant: name and priority %1 %2", listRec->name , listRec->priority)));
	...
	// xgettext:no-c-format
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::getLeastImportant: %1", leastImportant->name)));
	...
	if (numberOfLeastImportants < recList.size()) {
		...
		foreach(DvbSharedRecording listRecShared, recList)
		{
			...
			if (listRecShared->priority == leastImportance) {
				...
				// xgettext:no-c-format
				qDebug("%s", qPrintable(i18n("DvbRecordingModel::disableLeastImportants disabled: %1 %2", listRec.name , listRec.begin.toString())));
			}
		}
	}
...
void DvbRecordingModel::findNewRecordings()
{
		...
		foreach(QString regex, regexList) {
			...
			if (!recordingRegex.isEmpty())
				...
				if (recordingRegex.indexIn(title) != -1)
				{
					...
					// xgettext:no-c-format
					qDebug("%s", qPrintable(i18n("DvbRecordingModel::findNewRecordings: scheduled %1", title)));
					}
				}
			}
			i = i + 1;
	...
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::findNewRecordings executed.")));
}
...
void DvbRecordingModel::bindToSqlQuery(SqlKey sqlKey, QSqlQuery &query, int index) const
{
	...
	if (!recording.isValid()) {
		qWarning("%s", qPrintable(i18n("DvbRecordingModel::bindToSqlQuery: invalid recording")));
		return;
	}
...
int DvbRecordingModel::getSecondsUntilNextRecording() const
{
	...
	foreach (const DvbSharedRecording &recording, recordings) {
		...
		if (end > QDateTime::currentDateTime().toUTC() && rec.begin <= QDateTime::currentDateTime().toUTC()) {
			timeUntil = 0;
			// xgettext:no-c-format
			qDebug("%s", qPrintable(i18n("DvbRecordingModel::getSecondsUntilNextRecording: rec ongoing %1", rec.name)));
			break;
		}
	...
	// xgettext:no-c-format
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::getSecondsUntilNextRecording: returned TRUE %1", QString::number(timeUntil))));
	return timeUntil;
}
	...
	if (idleTime > 1000 * 3600) {
		if (DvbRecordingModel::getSecondsUntilNextRecording() > numberOfChannels * 10) {
			if (DvbRecordingModel::isScanWhenIdle()) {
				return true;
				qDebug("%s", qPrintable(i18n("DvbRecordingModel::shouldWeScanChannels: returned TRUE")));
			}
		}
	}
...
void delay(int seconds)
{
	...
	// xgettext:no-c-format
	qInfo("%s", qPrintable(i18n("DvbRecordingModel::delay: Delayed for %1 seconds", QString::number(seconds))));
}

void DvbRecordingModel::scanChannels()
{
	qDebug("%s", qPrintable(i18n("DvbRecordingModel::scanChannels executed")));
	...
	if (shouldWeScanChannels()) {
		...
		foreach (int channelInt, channelMap.keys()) {
			...
			if (channel.isValid()) {
				// TODO update tab
				// xgettext:no-c-format
				qDebug("%s", qPrintable(i18n("DvbRecordingModel::scanChannels executed %1", channel->name)));
...
bool DvbRecordingFile::start(DvbRecording &recording)
{
	...
	if (!file.isOpen()) {
		...
		for (int attempt = 0; attempt < 100; ++attempt) {
			...
			if (file.open(QIODevice::WriteOnly)) {
				...
				// xgettext:no-c-format
				qWarning("%s", qPrintable(i18n("DvbRecordingFile::start: cannot open file %1", file.fileName())));
			}
			...
			if ((attempt == 0) && !QDir(folder).exists()) {
				if (QDir().mkpath(folder)) {
					...
					// xgettext:no-c-format
					qWarning("%s", qPrintable(i18n("DvbRecordingFile::start: cannot create folder %1", folder)));
				}
			}
	...
	if (device == NULL) {
		...
		if (device == NULL) {
			qWarning("%s", qPrintable(i18n("DvbRecordingFile::start: cannot find a suitable device")));
			return false;
		}
