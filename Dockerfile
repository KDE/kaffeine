...
RUN cd ~/v4l-utils && ./bootstrap.sh && ./configure \
...
RUN wget https://linuxtv.org/downloads/dtv-scan-tables/kaffeine/scantable.dvb -O ~/kaffeine/src/scanfile.dvb
RUN cd ~/kaffeine && cmake . && make VERBOSE=1 && make install
...
RUN apt-get clean -y && rm -rf /var/lib/apt/lists/* ~/kaffeine ~/v4l-utils
